...
tinymce.PluginManager.add('vyai', function (editor) {
      ...
      return editor.windowManager.open({
        ...
        body: {
          ...
          items: [
            ...
            {
              type: 'htmlpanel',
              html: '<div style="margin-top: 10px; font-size: 12px; color: #666;">' + editor.translate('Selected text will be used as context') + '</div>',
            },
          ],
        },
        buttons: [
        ...
        onSubmit: function (api) {
          ...
          if (!VYAI || !VYAI.api_key) {
            editor.windowManager.alert(editor.translate('vyAI configuration is missing. Please check your setup.'));
            return;
          }
          ...
          getResponseFromOpenAI(currentPrompt, currentInput)
            ...
            .catch((error) => {
              ...
              editor.windowManager.alert(editor.translate('Error generating content: ') + error.message);
            });
        },
      });
    };
    ...
    const handleRetry = () => {
      ...
      const retryDialog = editor.windowManager.open({
        ...
        body: {
          ...
          items: [
            {
              type: 'htmlpanel',
              html: '<div style="text-align: center; padding: 20px;">' + editor.translate('Regenerating content with the same prompt...') + '</div>',
            },
          ],
        },
        buttons: [],
      ...
      getResponseFromOpenAI(currentPrompt, currentInput)
        ...
        .catch((error) => {
          ...
          retryDialog.close();
          editor.windowManager.alert(editor.translate('Error regenerating content: ') + error.message);
        });
    };
  ...
  /* Add a vyAI icon */
  editor.ui.registry.addIcon('vyai', '<img src="/icons8-chatbot-64.png" alt="vyAI" style="width: 20px; height: 20px;" />');
